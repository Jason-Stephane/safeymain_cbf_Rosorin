...
class HapticAudioEngine:
    def __init__(self,
        ...
        # Each "cycle" = 1/corridor_rate seconds split into left-on / gap / right-on / gap
        self._corridor_active  = False
        self._corridor_rate    = float(corridor_rate)  # full L→R cycles per second
        self._corridor_gap     = float(corridor_gap)   # fraction of half-cycle that's silent (0–0.5)
        self._corridor_base_L  = 0.0
        self._corridor_base_R  = 0.0
    ...
    # ========================= corridor cue (reusable) =========================
    def set_corridor(self, base_L, base_R):
        """
        Trigger the sample-accurate L/R ping-pong cue (same mechanism used for the
        proximity corridor case) at the given peak amplitudes. Any caller — not just
        set_multi_targets — can use this to signal "flanked on both sides".
        """
        self._corridor_active = True
        self._corridor_base_L = float(base_L)
        self._corridor_base_R = float(base_R)

    def clear_corridor(self):
        """Stop the ping-pong cue and fall back to normal ampL_target/ampR_target smoothing."""
        self._corridor_active = False
