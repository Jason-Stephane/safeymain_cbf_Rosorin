...
class SharedAutonomyController(Node):
    def __init__(self):
        ...
        self.declare_parameter('base_frame', 'base_link') #added to fix h vis
        self.base_frame = self.get_parameter('base_frame').value # added to fix h_vis
        ...
        # ----------------------------------------------------------------------------
        self.deadman_switch  = 1        # Button that must be held for motion to be enabled
        self.autonomy_switch = 1        # (Reserved) toggle for autonomy level
        ...
        self.get_logger().info(f'Using base_frame = {self.base_frame}') # troubleshooting h
    ...
    def publish_finalVelocity_marker(self):
       ...
       # Nearest obstacle only (single line, matches the CBF node's rho)
        if self.rho_values:
        ...
        # ONE combined log call — ref, rep, vfinal, then nearest rho
        self.get_logger().info(
            f"\n"
        ...
        for j in range(len(self.base_r_values)):
            if self.base_r_values[j] < self.marker_threshold_range:
                ...
                if points:
                    ...
                    if points:
                        dmin = min(math.hypot(p.x, p.y) for p in points)
                        dmax = max(math.hypot(p.x, p.y) for p in points)

                        self.get_logger().info(
                            f"Cluster {markerNumber}: "
                            f"{len(points)} pts | "
                            f"nearest={dmin:.2f} m | "
                            f"furthest={dmax:.2f} m"
                        )
